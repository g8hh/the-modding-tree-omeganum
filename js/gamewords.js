...
function generatetitle1() {
    let seed = Math.floor(Math.random() * 1)
    let words = [
        'aback',
        'abaft',
        'abandoned',
    ...
    switch (seed) {
        case 0:
            return words[Math.randomFloor(words.length)]
    }
}
function generatetitle2() {
    let seed = Math.floor(Math.random() * 1)
    let words = [
...
function generatetitle3() {
    let seed = Math.floor(Math.random() * 1)
    let words = [
        "idle", "incremental", "tycoon", "inc", "game", "clicker", "creator", "prestige", "mod", "maker",
        "dimensions", "layers", "infinity", "omega", "alpha", "beta", "tapper", "clan", "cult", "quest",
        "rpg", "journey", "kingdom", "creation", "civilization", "war", "city", "town", "world", "universe",
        "galaxy", "dimension", "planet", "farm", "factory", "miner", "developer", "gods"
    ]
    let chars = "abcdefghijklmnopqrstuvwxyz0123456789"
    let num = Math.random() > .5 ? Math.randomFloor(10000) : (Math.randomFloor(4) + 6).toString().repeat(Math.floor(Math.random() * 3 + 2))
    let len = Math.randomFloor(4) + 6
